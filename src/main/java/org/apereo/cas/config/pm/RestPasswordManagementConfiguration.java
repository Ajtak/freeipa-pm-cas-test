...
public class RestPasswordManagementConfiguration {
    ...
    private static RestTemplate buildRestTemplateBuilder(final RestTemplateBuilder restTemplateBuilder,
        ...
        if (StringUtils.isNotBlank(username) && StringUtils.isNotBlank(password)) {
            System.err.println("Configuring basic authentication for password management via REST for [{}]" + username);
            return restTemplateBuilder.basicAuthentication(username, password).build();
        }
        System.err.println("Basic authentication for password management via REST is turned off");
